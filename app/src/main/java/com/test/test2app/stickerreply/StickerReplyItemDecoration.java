package com.test.test2app.stickerreply;

import android.graphics.Rect;
import android.view.View;

import androidx.annotation.NonNull;
import androidx.recyclerview.widget.RecyclerView;

import com.test.test2app.fastrecordviewnew.UiUtils;

/**
 * created by zhaoyuntao
 * on 28/12/2020
 * description:
 */
class StickerReplyItemDecoration extends RecyclerView.ItemDecoration {
    private final int spaceHorizontal;
    private final int spaceVertical;

    public StickerReplyItemDecoration() {
        spaceHorizontal = UiUtils.dipToPx(3);
        spaceVertical = UiUtils.dipToPx(3);
    }

    @Override
    public void getItemOffsets(@NonNull Rect outRect, @NonNull View view, @NonNull RecyclerView parent, @NonNull RecyclerView.State state) {
        outRect.set(spaceHorizontal, spaceVertical, spaceHorizontal, spaceVertical);
    }
}
